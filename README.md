...
При очередном свайпе необходимо выдать следующего рекомендованного человека. Для этого необходимо произвести запрос за подходящими профилями и за свайпами, чтобы проверить полученные профили на повторения. Предположим, что данную информацию будем запрашивать пачками по 10 штук, т.е. 40 млн раз.

	Запросы на получение:

	40e6 / (24 * 60 * 60) ≈ 463 RPS
...
#### Профили
	...
	Также запросы для рекомендаций:
...
|Сервис мэтчей и рекомендаций|32|128|256|3 + 6|
