...
|6 млн (~50%)|[ Количество пользователей в пиковое время (21:00-22:00)](https://www.huffingtonpost.co.uk/entry/how-to-get-tinder-matches_n_56a78f4be4b0172c659422da)|
...
При среднем времени использования сервиса в день равным 25 минутам и количеcтве пользователей, ежедневно пользующиеся сервисом, равным 12 млн:
	...
	Статика - 600 КБ * 25 * 12e6 ≈ 172 000 Гбайт/сутки
	
	Динамика - 120 КБ * 25 * 12e6 ≈ 34 334 Гбайт/сутки
	
	Изображения - 3 МБ * 25 * 12e6 ≈ 879 900 Гбайт/сутки
...
В пиковое время (21:00-22:00) количество пользователей составляет 6 млн. Пусть среднее время использования сервиса за этот час составляет 10 минут на пользователя и использование сервиса распределено равномерно. Тогда для 60 пользователей пик составит 10 одновременных сессий. Для 6 млн - 1 млн.
	...
	Статика - 600 КБ * 1e6 / 60 ≈ 76 Гбит/сек
...
При добавлении новой фотографии в файловой системе сервера будет создаваться директория c именем, содержащим уникальный сгенерированный хеш (SHA256) переданной фотографии. В этой директории будет хранится переданное изображение в оригинальном и измененных размерах (640x800, 320x400, 172x216, 84x106)
...
Фотографии будем шардировать по url-у, т.е. по имени директории, в которой хранятся изображения. Распределять будем по алфавиту a-f - 1, e-h - 2, ..., 3-10 - n. Это позволит равномерно распределить фото по шардам.

В url-е не будет содержаться ip сервера. В nginx с помощью регулярок будем анализировать запрашиваемый url и перенаправлять запрос на необходимый сервер в соответствии со схемой шардинга, описанной выше.
...
#### Информация о фотографиях
	...
	Требуемое оборудование для профилей и фотографий:
	
	(89 ГБ + 6 ГБ) / 256 ГБ -> 1 шт
...
#### Мэтчи
	...
	Требуемое оборудование для свайпов и мэтчей:
	
	(279 ГБ + 2.1 ГБ) / 128 ГБ ≈ 2.2 -> 3 шт / мес
...
Примем среднюю длину сообщения равную 30 Б.
	...
	Требуемое оборудование для чатов и сообщений:
	
	(1.2 ГБ + 81 ГБ) / 256 ГБ ≈ 0.32 -> 1 шт / мес
...
| -------------  |  :-------------:  | :-------------:  | :-------------:  | :-------------:  | :-------------:  |
|Профили пользователей|PostgresSQL|id|94 ГБ|80 млн|1|
|Информация о фотографиях| PostgresSQL |user\_id|6 ГБ|240 млн||
|Сессии пользователей|Redis|user\_id|0.3 ГБ|33 млн|1|
|Свайпы|Redis|user\_src\_id|279 ГБ / мес|12 млрд / мес|3 шт / мес|
|Мэтчи|Redis|id|2.1 ГБ / мес|90 млн / мес ||
|Сообщения|PostgresSQL|chat\_id|81 ГБ / мес|1.260 млрд / мес|1 шт / мес|
|Чаты|PostgresSQL|id|1.2 ГБ / мес|63 млн / мес||
...
|Сервис чатов|32|128|256|1 + 2|
|Сервис пользователя / рекомендаций / свайпов и мэтчей|32|128|256|4 + 8|
|БД PostgreSQL|32|128|256|(1 + 2) + <br> (1 + 2 шт / мес)|
|БД Redis|32|128|256|(1 + 2) + <br> (3 + 6 шт / мес)|
